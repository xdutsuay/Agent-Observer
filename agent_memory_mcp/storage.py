...
class Storage:
    ...
    def generate_context_file(self, repo_id: str, project_path: str | Path) -> Path:
        """Generate a .agent-memory/context.md for a project that IDEs auto-load."""
        ...
        top = self.engine.list_memories(repo_id=repo_id, limit=50)
        top = [
            m for m in top
            if (m.get("quality_tier") or "unrated") != "noise"
        ]
        top.sort(key=lambda m: m.get("relevance_score", 0) or 0, reverse=True)
        top = top[:10]

        now = datetime.now(timezone.utc).isoformat()
        parts = [
            f"# Agent Memory Context (auto-generated)",
            f"# Project: {repo_id}",
            f"# Last updated: {now}",
            f"# Do not edit — regenerated by agent-memory-mcp\n",
        ]

        if ctx.get("decisions") and ctx["decisions"] != "(none)":
            parts.append(f"## Key Decisions\n{ctx['decisions']}\n")

        if ctx.get("failures") and ctx["failures"] != "(none)":
            parts.append(f"## Known Issues\n{ctx['failures']}\n")

        if ctx.get("facts") and ctx["facts"] != "(none)":
            parts.append(f"## Project Facts\n{ctx['facts']}\n")

        if top:
            parts.append("## High-Relevance Memories")
            for m in top:
                score = m.get("relevance_score", 0) or 0
                parts.append(f"- [{m['kind']}] (score: {score:.2f}) {m['content'][:200]}")
            parts.append("")
